...
const Page = () => {
  ...
  // Removed downloadBrokenPDF from handleSubmit scope to prevent nesting errors
  const downloadBrokenPDF = async () => {
    ...
    // Footer Panel Documentation Layout Loop
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setResult(null); // Clear previous result records
    setSitemapMissing(false); // Reset validation warnings
    setLoading(true);
    ...
    try {
      ...
      // Check: If sitemap does not exist
      if (res.data.sitemapExists === false) {
        setSitemapMissing(true); // Render sitemap layout alert block
        setResult(null); // Keep results hidden from layout view
      } else {
        // Render result dashboard when a sitemap is successfully validated
        setResult(res.data);
        setSitemapMissing(false);
      }

    } catch (error) {
      alert("Scan failed. Please check the target URL.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <main className="m-8">
      ...
      <section className="mt-4 bg-gray-100 p-4 rounded-2xl">
        ...
        <ul className="mt-2 space-y-2 pl-2">
          <li>
            <h4 className="text-lg md:text-xl font-semibold font-['Nunito']">404 Error (Page Not Found)</h4>
            <p className="text-base md:text-lg font-['PT Sans']">The requested page does not exist on the server.</p>
          </li>
          <li>
            <h4 className="text-lg md:text-xl font-semibold font-['Nunito']">Server Errors (500 Errors)</h4>
            <p className="text-base md:text-lg font-['PT Sans']">Indicates server issues that prevent page loading.</p>
          </li>
          <li>
            <h4 className="text-lg md:text-xl font-semibold font-['Nunito']">Redirect Errors</h4>
            <p className="text-base md:text-lg font-['PT Sans']">Improper or broken redirections leading to inaccessible pages.</p>
          </li>
          <li>
            <h4 className="text-lg md:text-xl font-semibold font-['Nunito']">Timeout / Unreachable Links</h4>
            <p className="text-base md:text-lg font-['PT Sans']">Links that fail to respond or take too long to load.</p>
          </li>
        </ul>
      </section>
